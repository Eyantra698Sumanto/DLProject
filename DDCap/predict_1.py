#CUDA_VISIBLE_DEVICES=0 python -m torch.distributed.launch --nproc_per_node 1  predict_1.py --is_rn --normalize_prefix --bs=128  --data ~/dl/DDCap/data/coco/oscar_split_ViT-B_32_train_512.pkl --epochs 30 --out_dir ~/dl/DDCap/results_diff3 --tag caption_diff_vitb16
import os
import torch
...
class ClipCocoDataset(Dataset):
    ...
    def __getitem__(self, item: int) -> Tuple[torch.Tensor, ...]:
        ...
        if not(os.path.isfile(filename)):
            filename = f"{self.data_root}/train2014/COCO_train2014_000000311081.jpg"
        try:
            image = io1.imread(filename)
        except:
            filename = f"{self.data_root}/val2014/COCO_val2014_{int(img_id):012d}.jpg"
            if not(os.path.isfile(filename)):
                filename = f"{self.data_root}/val2014/COCO_val2014_000000311081.jpg"
            image = io1.imread(filename)
        # print(filename)
...
class ClipCocoValDataset(Dataset):
    ...
    def __init__(self, data_root: str):
        ...
        self.annotation = json.load(open("./MSCOCO_Caption/annotations/captions_predict.json", "r"))["annotations"]
        with open('coco_predict.txt') as f:
...
def val(model, epoch, val_dataloader, args):
    ...
    if dist.get_rank() == 0:
        ...
        result = evaluate_on_coco_caption(result_all,
                                          ...
                                          os.path.join(args.data_root, 'annotations/captions_predict.json'))
    else:
        result = None
    torch.distributed.barrier()
    if dist.get_rank() == 0:
...
def main(args):
    ...
    ckpt = torch.load('/home/22m1526/dl/DDCap/results_diff3/caption_diff_vitb16/caption_diff_vitb16-006.pt', map_location="cpu")
...
if __name__ == '__main__':
    ...
    if 'RANK' in os.environ and 'WORLD_SIZE' in os.environ:
        ...
        rank = -1
        world_size = -1
    dist.init_process_group("nccl", init_method='tcp://localhost:1998', rank=args.local_rank, world_size=world_size)
    torch.distributed.barrier()
