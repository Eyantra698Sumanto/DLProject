...
class ClipCocoDataset(Dataset):
    ...
    def __getitem__(self, item: int) -> Tuple[torch.Tensor, ...]:
        ...
        if not os.path.exists(filename):
            filename = f"{self.data_root}/train2014/COCO_train2014_000000144983.jpg"
            img_id = "000000144983"
...
if __name__ == '__main__':
    ...
    if 'RANK' in os.environ and 'WORLD_SIZE' in os.environ:
        ...
        rank = -1
        world_size = -1
    dist.init_process_group("nccl", init_method='tcp://localhost:1999', rank=args.local_rank, world_size=world_size)
    torch.distributed.barrier()
